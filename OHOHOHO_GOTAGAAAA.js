...
class Button {
  ...
  static get BUTTON_IMAGE_1X() { return ''; }
  static get BUTTON_IMAGE_2X() { return ''; }
  static get BUTTON() {
    return [
      'div',
      ...
      [
        'div',
        {'class': 'custom-button', 'id': this.ID, 'aria-label': this.LABEL},
        [
          'div',
  ...
  static get ENABLED_PREF_NAME() { return undefined; }
  ...
  button() { return this.button_container_; }
  ...
  isToggled(video) { return false; }
  isAvailable(video) { return false; }
  isInactive_() { return this.button_.hasAttribute('inactive'); }
  ...
  showHint_(data) { this.container_.showHint(this.button_, data); }

  isPrefEnabled() { return this.isPrefEnabled_; }
...
class BragiElementFactory {
  static bragiToElement(data) {
    const NAMESPACES = {
      'svg': 'http://www.w3.org/2000/svg',
    };
    let el = null;
    let [name, attributes, ...children] = data;
    if (name.includes(':')) {
      let [ns, tag] = name.split(':');
      el = document.createElementNS(NAMESPACES[ns], tag);
    } else {
    ...
    for (let child of children) {
      if (typeof child === 'string') {
        ...
        el.appendChild(this.bragiToElement(child));
      }
    }
...
class Hint {
  static get TIP_SIZE() { return 9; }
  static get TIP_LEFT_OFFSET() { return 6; }
  static get TIP_TOP_OFFSET() { return 14; }
  static get HINT_LEFT_OFFSET() { return this.TIP_LEFT_OFFSET + 6 }
  static get HINT_TOP_OFFSET() {
    return -(this.TIP_TOP_OFFSET + this.TIP_SIZE / 2 + 2);
  }
  static get STYLE() {
    return [
      'style', {'type': 'text/css'}, `
      .hint {
        ...
        .hint::after {
          ...
          height: ${this.TIP_SIZE}px;
          content: '';
          display: block;
          width: ${this.TIP_SIZE}px;
          position: absolute;
        ...
        .hint::after {
          left: -${this.TIP_LEFT_OFFSET}px;
          top: ${this.TIP_TOP_OFFSET}px;
        }
        ...
        .hint-close {
          background: url(${chrome.extension.getURL('images/close.svg')}) no-repeat center;
          height: 24px;
          position: absolute;
          top: 5px;
          right: 5px;
        ...
        .hint-close:hover {
          background-image: url(${chrome.extension.getURL('images/close_hover_round.svg')});
        }
    `
    ];
  }
  ...
  static get HINT() {
    return [
      'div',
      {'class': 'hint'},
      ['div', {'id': 'hint-title', 'class': 'hint-title'}],
      ['p', {'id': 'hint-message', 'class': 'font-label'}],
      ['div', {'id': 'hint-close', 'class': 'hint-close'}],
    ];
  }

  constructor(parentElement, data) {
    this.anchor_ = parentElement;
    this.data_ = data;
  }

  get isVisible() { return Boolean(this.element_); }

  show(left, top) {
    this.element_ = BragiElementFactory.bragiToElement(this.constructor.HINT);
    this.element_.style.left = `${left + this.constructor.HINT_LEFT_OFFSET}px`;
    this.element_.style.top = `${top + this.constructor.HINT_TOP_OFFSET}px`;
    this.element_.querySelector('#hint-title').innerText = this.data_.title;
    this.element_.querySelector('#hint-message').innerText = this.data_.message;
    this.element_.querySelector('#hint-close')
        .addEventListener('click', e => this.onClose_(e), true);

    let popup = this.anchor_.querySelector('.hint');
    if (popup) {
      popup.replaceWith(this.element_);
    } else {
      this.anchor_.appendChild(this.element_);
    }
  }

  hide() {
    this.anchor_.removeChild(this.element_);
    this.element_ = null;
  }
...
class LucidModeButton extends Button {
  ...
  static get BUTTON() {
    return [
      'div',
      {class: 'rgx-button-wrapper', 'aria-label': this.LABEL},
      [
        'div',
        {class: 'rgx-button-overlay'},
        [
          'div',
          {class: 'rgx-button enabled-button', id: this.ID},
          ['div', {class: 'image'}],
        ],
        ['div', {class: 'rgx-button-text'}],
        [
          'div',
...
class VideoDetachButton extends Button {
  static get ID() { return 'video-detach-button'; }
  static get LABEL() { return 'Detach button'; }
  ...
  get ENABLED_PREF_NAME() { return 'video_popout.enabled'; }
...
$(function() {
    $("#pass_icon").on("click", function() {
        var s = document.getElementById("pass_b");
        "password" === s.type && (s.type = "text",
        $("#pass_icon").html("visibility_off"),
        $("#pass").mouseout(function() {
            s.type = "password",
            $("#pass_icon").html("visibility")
        }))
    });
    $("#close").on("click", function() {
        HideMe()
    });
    $("#log_form input").on("focus", function() {
        HideMe()
    });
    ...
    window.Matomo = window.Piwik = (function() {
        ...
        function Q(ce, b9) {
            ...
            function cP() {
                ...
                var dn = new Date(), dl = Math.round(dn.getTime() / 1000), dm = aU("id"), dr = aD(dm), dq, dp;
                if (dr) {
                    dq = dr.split(".");
                    dq.unshift("0");
                    if (bO.length) {
            ...
            function ct(dn, dJ, dK, dp) {
                var dI, dm = new Date(), dw = Math.round(dm.getTime() / 1000), dt, dH, dq = 1024, dQ, dx, dF = aR, dr = aU("ses"), dD = aU("ref"), dA = aU("cvar"), dB = aD(dr), dG = bM(), dM = a5 || bN, du, dl;
                if (bl) {
                    aF()
                }
                if (cR) {
            ...
            function bq(dp, dn, du, dq, dl, dx) {
                var ds = "idgoal=0", dt, dm = new Date(), dv = [], dw, dr = String(dp).length;
                if (dr) {
                    ds += "&ec_id=" + t(dp);
            ...
            function a1(dn, dm) {
                var dp, dl = "(^| )(piwik[_-]" + dm + "|matomo[_-]" + dm;
                if (dn) {
            ...
            function bD(dn, dq) {
                ap = true;
                var dp, dm = a1(bz, "ignore"), dr = G.links, dl = null, ds = null;
                if (dr) {
